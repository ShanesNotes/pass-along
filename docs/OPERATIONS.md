...
| `SUPABASE_JWT_SECRET` | Verifying Supabase-issued admin JWTs on `/api/admin/*`. | `/api/admin/*` falls back to `ADMIN_DEMO_TOKEN` bearer auth; if that's also unset, `503 ADMIN_DISABLED`. | Secret. HS256 signing key — treat like a password, never expose to browser bundles. |
...
| `ADMIN_DEMO_TOKEN` | Accessing `/api/admin/queue` and `/api/admin/decide` when `SUPABASE_JWT_SECRET` is unset. | Admin APIs return `503 ADMIN_DISABLED`; raw stories stay unavailable. | Secret demo bearer token. Rotate after demos; replace with `SUPABASE_JWT_SECRET` before production. |
...
- Keep `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_JWT_SECRET`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `GEMINI_API_KEY`, and `ADMIN_DEMO_TOKEN` server-side only.
- `/api/admin/*` picks its auth method by what's configured: `SUPABASE_JWT_SECRET` set → verify bearer JWTs (HS256, admin claim required); else `ADMIN_DEMO_TOKEN` set → bearer token match; else `503 ADMIN_DISABLED` (fail-closed, matching the existing default).
