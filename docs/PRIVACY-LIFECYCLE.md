...
`LifecycleStoragePort` (`packages/engine/src/lifecycle/ports.ts`) is the seam: an in-memory implementation (`memory.ts`) backs the unit tests, and `PgLifecycleStorage` (`pg-adapter.ts`) implements the same port against the tables above with plain SQL — no schema changes were needed beyond `0004_data_rights_requests.sql`. `PgPassStore` (`packages/engine/src/db/adapters/pass-store.ts`) composes `PgLifecycleStorage` alongside its job/intake storage, so `hasLifecycleStorage()` is true for the real DB-backed store `createStores()` hands out — the `deleteSubmission` job's cascade runs for real once `DATABASE_URL` is set, not just against the in-memory demo store. Auth is still the gate on *triggering* a deletion: only an authenticated admin action (PA-025's `AdminAuthenticator`) should be able to fire the `moderation.decided(action: "remove")` event that this job listens for.
