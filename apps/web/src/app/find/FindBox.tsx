...
/** The live find pipeline's interactive surface — textarea, submit, and every
 * response state (loading/error/clarify/crisis/results). Shared by the /find
 * page and the product landing page so submitting behaves identically. */
export function FindBox({
  ...
  if (view.kind === "crisis") {
    return <CrisisCard support={view.support} onBack={onBack} />;
  }
  ...
  return (
    <div className="pa-card">
      ...
      {view.kind === "clarify" && (
        <Clarify question={view.question} chips={view.chips} onClarifyChip={onClarifyChip} />
      )}
  ...
  return (
    <div className="pa-card-sm" style={{ marginTop: "1rem" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {chips.map((chip) => (
          <button key={chip} className="pa-chip" onClick={() => onClarifyChip(chip)}>
    ...
    <div style={{ marginTop: "1.5rem" }}>
      ...
      {sparse && (
        <div className="pa-notice">
          ...
          <Link href="/directions#direction-b" style={{ fontWeight: 700 }}>
